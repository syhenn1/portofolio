...
export default function Waves({
    className = "",
    strokeColor = "var(--line)",  // Subtle theme-aware lines for the portfolio
    backgroundColor = "transparent",       // Transparent background to layer correctly
    pointerSize = 0.5
}: WavesProps) {
    ...
    const setLines = () => {
        ...
        const xGap = 16  // Spacing increased for 4x rendering optimization
        const yGap = 16  // Spacing increased for 4x rendering optimization
    ...
    const onMouseMove = (e: MouseEvent) => {
        ...
        updateMousePosition(e.pageX / zoom, e.pageY / zoom)
    }
    ...
    const onTouchMove = (e: TouchEvent) => {
        ...
        if (touch) {
            const zoom = parseFloat(window.getComputedStyle(document.documentElement).zoom) || 1
            updateMousePosition(touch.clientX / zoom, touch.clientY / zoom)
        }
    }
    ...
    // Initialization and Animation Loop Control via IntersectionObserver (Pauses loop when offscreen)
    useEffect(() => {
        if (!containerRef.current || !svgRef.current) return
